...
RSpec.describe Search::AiRateLimiter do
  around do |example|
    original_semantic_limit = Rails.application.config.x.ai_search.semantic_rate_limit_per_minute
    original_answer_limit = Rails.application.config.x.ai_search.answer_rate_limit_per_minute
    original_window = Rails.application.config.x.ai_search.rate_limit_window_seconds
    Rails.application.config.x.ai_search.semantic_rate_limit_per_minute = 1
    Rails.application.config.x.ai_search.answer_rate_limit_per_minute = 1
    ...
    Rails.cache.clear
    Rails.application.config.x.ai_search.semantic_rate_limit_per_minute = original_semantic_limit
    Rails.application.config.x.ai_search.answer_rate_limit_per_minute = original_answer_limit
  ...
  it "limits semantic search operations per user/workspace window" do
    user = User.create!(email: "ai-rate-user@example.com", password: "password123")
  ...
  it "limits answer generation independently from semantic search" do
    user = User.create!(email: "ai-rate-answer@example.com", password: "password123")
