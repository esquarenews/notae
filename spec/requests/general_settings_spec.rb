...
RSpec.describe "General settings", type: :request do
  ...
  it "renders workspace controls in general settings" do
    ...
    expect(response.body).to include("Workspace games")
    expect(response.body).to include("The Archive")
    expect(response.body).to include("/w/#{workspace.slug}/_archive")
    expect(response.body).to include(workspace_archive_game_path(workspace_slug: workspace.slug))
    expect(response.body).to include("Nota Maze")
    expect(response.body).to include("/w/#{workspace.slug}/_nota_maze")
    expect(response.body).to include(workspace_nota_maze_game_path(workspace_slug: workspace.slug))
  ...
  it "prevents non-admin members from creating or downloading workspace exports" do
    ...
    get workspace_general_settings_path(workspace_slug: workspace.slug)
    expect(response).to have_http_status(:ok)
    expect(response.body).not_to include(workspace_export.token)
    expect(response.body).not_to include(owner.email)

    expect do
      post workspace_backup_exports_path(workspace_slug: workspace.slug)
    end.not_to change(WorkspaceExport, :count)
    expect(response).to redirect_to(root_path)
