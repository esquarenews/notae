...
RSpec.describe "Archive game", type: :request do
  it "renders the hidden archive game for workspace members" do
    ...
    expect(response.body).to include("notae-archive-game-screen-flash")
    expect(response.body).to include("notae-archive-game-event-cue")
    ...
    expect(response.body).to include('data-archive-game-target="screenFlash"')
    expect(response.body).to include('data-archive-game-target="eventCue"')
