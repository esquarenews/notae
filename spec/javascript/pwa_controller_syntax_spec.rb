...
RSpec.describe "PwaController JavaScript syntax" do
  ...
  it "manages install prompts, offline state, push subscriptions, and private cache clearing" do
    ...
    expect(source).to include("delivery_failed")
    expect(source).to include("Refreshing this device subscription")
    expect(source).to include("rebuildPushSubscription()")
    expect(source).to include("shouldRetryCurrentDeviceTestPush(response, payload)")
    expect(source).to include("sendTestPushRequest(path, endpoint, scope = \"all\")")
    expect(source).to include("sendTestPushRequest(path, refreshedSubscription.endpoint, \"current_endpoint\")")
    ...
    expect(source).to include("Test push reached this browser and was routed to the in-app notification bar.")
    ...
    expect(source).to include("Test push was accepted for this browser. It will appear in the notification bar while Notae is open.")
    ...
    expect(source).to include("notificationSuppressedInForeground")
    expect(source).not_to include("showIncomingPushToast")
    expect(source).not_to include("showPushLiveBanner")
    expect(source).not_to include("requestForegroundBrowserNotification")
