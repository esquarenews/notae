...
RSpec.describe "Notae Google Meet extension JavaScript syntax" do
  ...
  it "keeps the extension wired for popup state, transcript capture, and transcript upload" do
    ...
    expect(content).to include('[role="log"]')
    expect(content).to include("your meeting's ready")
    expect(content).to include("jump to bottom")
    expect(content).to include("stripSystemFragments")
    expect(content).to include("incrementalExtension")
    expect(content).to include("recentSignatureWindowMs")
    expect(content).to include("you\\s+just\\s+then")
    expect(content).to include("meet\\.google\\.com")
