...
RSpec.describe "NotificationBarController JavaScript syntax" do
  ...
  it "updates the shell clock and persists dismiss and snooze state for alerts" do
    ...
    expect(source).to include("if (this.hasAlertsTarget) {")
    ...
    expect(source).to include("showStoppedTimesheetTimer(detail = {})")
    expect(source).to include("Time sheet stopped")
    expect(source).to include("is-stopped")
    expect(source).to include("is-fading")
    expect(source).to include("stoppedTimesheetElapsed(detail = {})")
    expect(source).to include("detail.elapsedLabel")
    expect(source).to include("hideTimesheetTimer()")
    expect(source).to include("renderTimesheetTimer()")
    expect(source).to include("syncTimesheetTimerFromPage()")
    expect(source).to include("activeTimesheetClockInput()")
    expect(source).to include(".notae-timesheet-clock-cell input[data-timesheet-clock-role='start']")
    expect(source).to include("updateTimesheetTimerFromPayload(payload?.data?.active_timesheet_timer)")
    expect(source).to include("this.timesheetTimerTarget.classList.contains(\"is-stopped\")")
