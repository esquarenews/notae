module AgentActions
  class PreviewBuilder
    ...
    def to_h
      after_snapshot = normalize_snapshot(title: agent_action.title, payload: agent_action.payload)
      before_snapshot = previous_snapshot

      {
        "mode" => before_snapshot.present? ? "update" : "create",
        "before" => before_snapshot,
        "after" => after_snapshot,
        "changes" => build_changes(before_snapshot: before_snapshot, after_snapshot: after_snapshot)
      }
    end
    ...
    def normalize_snapshot(title:, payload:)
      field_definitions.map do |key, label|
        {
          "key" => key,
          "label" => label,
          "value" => normalized_value(key, title: title, payload: payload)
        }
      end
    end
    ...
    def field_definitions
      FIELD_DEFINITIONS.fetch(agent_action.draft_type, [])
    end

    def normalized_value(key, title:, payload:)
      raw_value =
        if key == "title"
          title
        else
