class KalendariumEventPolicy < ApplicationPolicy
  def show?
    calendar_policy.show?
  end

  def create?
    calendar_policy.update? && record.kalendarium_calendar.user_writable?
  end

  def update?
    create?
  end

  def destroy?
    update?
  end

  class Scope < Scope
    def resolve
      return scope.none unless user

      calendar_ids = KalendariumCalendarPolicy::Scope.new(user, KalendariumCalendar).resolve.select(:id)
      scope.where(workspace_id: accessible_workspace_ids, kalendarium_calendar_id: calendar_ids)
    end
  end

  private

  def calendar_policy
    @calendar_policy ||= KalendariumCalendarPolicy.new(user, record.kalendarium_calendar)
  end
end
